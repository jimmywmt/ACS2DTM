...
public class App {
  ...
  HashMap<String, Double> ahOutputDeletedTransactions;
  ...
  public LinkedList<int[]> ahGlobalBest;
  public double ahGlobalBestFitness;
  ...
  public void setFrequentItemsets(ArrayList<LinkedList<int[]>> frequentItemsets) {
    this.frequentItemsets = frequentItemsets;
  }

  public void setPreLargeFrequentItemsets(ArrayList<LinkedList<int[]>> preLargeFrequentItemsets) {
    preLargeFrequentItemsets.forEach(e -> e.forEach(preLarge::add));
  }
  ...
  public void printSensitiveItemsets() {
    ...
    sensitive.forEach(
        e -> System.out.println(Arrays.toString(e) + " : " + apriori.getItemsetCount(e)));
  }

  public void printNonSensitiveItemsets() {
    System.out.println("NonSensitive Itemsets");
    nonSensitive.forEach(
        e -> System.out.println(Arrays.toString(e) + " : " + apriori.getItemsetCount(e)));
  }
  ...
  private boolean containArray(int[] a1, int[] a2) {
    ...
    for (i = 0; i < a2.length; ++i) {
      if (Arrays.binarySearch(a1, a2[i]) < 0) break;
    }
  ...
  private void init(Apriori apriori) {
    ...
    ListIterator<int[]> it = sensitive.listIterator();
    while (it.hasNext()) {
      int count = countTable.get(Apriori.idArrayString(it.next()));
      if (count > maxSensitiveCount) maxSensitiveCount = count;
    }
    maxSelection = apriori.getMaxDelete();
    //    maxSelection =
    //        (int)
    //            ((Math.floor((maxSensitiveCount - apriori.minSupCount) / (1 - minSup)) + 1)
    //                * maxSelectionTimes);
    globalBestFitness = Double.MAX_VALUE;
    ahGlobalBestFitness = Double.MAX_VALUE;
  }
  ...
  private int countContain(int[] transaction, LinkedList<int[]> itemsets) {
    Integer count = 0;
    ListIterator<int[]> it = itemsets.listIterator();
    while (it.hasNext()) {
      if (containArray(transaction, it.next())) ++count;
    }
    return count;
  }
  ...
  private void setWeightsForRemainNodes() {
    ...
    remainNodes.forEach(
        (key, value) -> {
          ...
          double heuristic =
              alpha * (double) countNS + gamma * (double) countFF - beta * (double) countHF;
          ...
          if (weight > bestWeight) {
            ...
            if (bestWeightKey.split(",").length > key.split(",").length) {
              ...
              ++bestCount;
              if (Math.random() > (1.0 / (double) bestCount)) bestWeightKey = key;
            }
          }
        });
  }
  ...
  private double calculateNodeFitnessAndRemoveNode(int[] transaction) {
    ...
    //    int minSupCount = (int) Math.ceil((apriori.transactionsCount() - current.size()) *
    // minSup);
    ...
    for (int[] itemset : sensitive) {
      ...
      if (count
          >= (int)
              Math.ceil(
                  (apriori.transactionsCount() - current.size())
                      * apriori.dynamicMinSup(itemset.length))) nonHideSensitive.add(itemset);
    }
    ...
    for (int[] itemset : nonSensitive) {
      if (containArray(transaction, itemset)) {
        ...
        if (count
            < (int) Math.ceil(apriori.transactionsCount() - current.size())
                * apriori.dynamicMinSup(itemset.length)) hideFrequent.add(itemset);
        itemsetsCount.put(itemString, count);
      }
    }
    ...
    for (int[] itemset : preLarge) {
      ...
      if (count
          >= (int) Math.ceil(apriori.transactionsCount() - current.size())
              * apriori.dynamicMinSup(itemset.length)) fakeFrequent.add(itemset);
    }

    remainNodes.remove(Apriori.idArrayString(transaction));

    return alpha * nonHideSensitive.size()
        + beta * hideFrequent.size()
        + gamma * fakeFrequent.size();
  }

  public void runIteration(boolean recordProjectedDataset) {
    LinkedList<int[]> iterationBest = new LinkedList<>();
    ...
    for (int i = 0; i < populationSize; ++i) {
      ...
      for (int j = 0; j < maxSelection; ++j) {
        ...
        if (Math.random() <= q0) selectKey = bestWeightKey;
        else {
          double ball = totalWeight * Math.random();
          Iterator<Map.Entry<String, Double>> it = remainNodes.entrySet().iterator();
          while (it.hasNext()) {
        ...
        double fitness =
            calculateNodeFitnessAndRemoveNode(
                Arrays.stream(selectKey.split(",")).mapToInt(Integer::parseInt).toArray());

        if ((fitness < iterationBestFitness)
            || ((fitness == iterationBestFitness) && (current.size() < iterationBest.size()))) {
          iterationBestFitness = fitness;
          iterationBest = new LinkedList<int[]>(current);
        }
        if ((fitness < globalBestFitness)
            || ((fitness == globalBestFitness) && (current.size() < globalBest.size()))) {
          globalBestFitness = fitness;
          globalBest = new LinkedList<int[]>(current);
          fth = nonHideSensitive.size();
          nth = hideFrequent.size();
          ntg = fakeFrequent.size();
          if (recordProjectedDataset)
            outputDeletedTransactions = new HashMap<String, Double>(remainNodes);
        }
        if (nonHideSensitive.size() == 0
            && ((fitness < ahGlobalBestFitness)
                || ((fitness == ahGlobalBestFitness) && (current.size() < ahGlobalBest.size())))) {
          ahGlobalBestFitness = fitness;
          ahGlobalBest = new LinkedList<int[]>(current);
          ahfth = nonHideSensitive.size();
          ahnth = hideFrequent.size();
          ahntg = fakeFrequent.size();
          if (recordProjectedDataset)
            ahOutputDeletedTransactions = new HashMap<String, Double>(remainNodes);
        }
      }
    }
    double addPhoromone = (maxFitness - iterationBestFitness) / maxFitness;
    iterationBest.forEach(
        e -> {
  ...
  public static void main(String[] args) {
    ...
    options.addOption("mm", true, "Minmal Value of Minimum Support");
    ...
    try {
      ...
      if (cmd.hasOption("mm")) {
        minValueOfMinSup = Double.parseDouble(cmd.getOptionValue("mm"));
        System.out.println("The minimal value of minimum support is " + minValueOfMinSup);
      } else {
        hf.printHelp("java -jar ACS2DT.jar", options, true);
        exit(1);
      }
    ...
    ants.minSup = minSup;
    ants.minValueOfMinSup = minValueOfMinSup;
    ants.alpha = alpha;
    ants.beta = beta;
    ants.gamma = gamma;
    ants.readSensitiveInformation(sensitiveFileName);
    ants.maxSelectionTimes = maxSelectionTimes;
    apriori.setMaxSelectionTimes(maxSelectionTimes);
    apriori.setMinSup(minSup);
    apriori.setMinValueOfMinSup(minValueOfMinSup);
    apriori.calculateMaxDelete(ants.sensitive);
    apriori.run();
    ants.init(apriori);
    System.out.println(
        "The minimal size is :" + (apriori.getTransactions().size() - ants.maxSelection));
    ...
    if (ants.ahGlobalBestFitness != Double.MAX_VALUE) {
      System.out.println("Find all hide solution!");
      System.out.println("\nfitness\tF-T-H\tN-T-H\tN-T-G\tsize");
      System.out.print(ants.ahGlobalBestFitness + "\t");
      System.out.print(ants.ahfth + "\t");
      System.out.print(ants.ahnth + "\t");
      System.out.print(ants.ahntg + "\t");
      System.out.println(apriori.transactionsCount() - ants.ahGlobalBest.size() + "\n");
    }
    System.out.println(
    ...
    if (recordProject) {
      ...
      try {
        ...
        ants.outputDeletedTransactions
            .entrySet()
            .forEach(
                e -> {
                  try {
